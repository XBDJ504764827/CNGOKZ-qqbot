...
func TestReplyText(t *testing.T) {
	cases := []struct {
		...
		{"bound", lumiadmin.BindOutcome{Result: "bound"}, "绑定成功！请返回网站继续下一步。"},
...
// 避免玩家私发验证码后一直等待（handled=true 表示该消息已被消费）。
func TestBinderDisabledRepliesToCode(t *testing.T) {
	b := NewBinder(nil, nil, zap.NewNop())
	if handled, err := b.HandleC2CMessage(context.Background(), "openid-1", "玩家", "绑定 WL-7KQ2XA"); !handled || err != nil {
		t.Fatalf("HandleC2CMessage() = (%v, %v), want (true, nil)", handled, err)
	}
	// 非验证码消息不应被消费
	if handled, _ := b.HandleC2CMessage(context.Background(), "openid-1", "玩家", "你好"); handled {
		t.Fatal("普通消息不应被消费")
	}
}
