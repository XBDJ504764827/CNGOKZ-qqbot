...
LumiBot 是通知机器人：所有事件通知均为**纯文本推送**，不附带按钮、不提供按钮审批；审批与业务操作全部在 LumiAdmin 后台完成。白名单申请支持 QQ 私聊绑定：玩家添加机器人 QQ 后**私聊发送网站验证码**即可完成 Steam↔QQ 绑定，管理员也可在后台私聊玩家（见 docs/API.md）。
